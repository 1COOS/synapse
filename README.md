...
桌面端点击左栏底部齿轮按钮进入“模型设置”。应用会优先把 `apiKey` 存到系统安全存储中，`baseUrl` 和模型名保存在本地配置文件；如果 macOS 当前是无开发证书的未签名 Debug 构建，Keychain 可能返回 `-34018`，此时会降级把 key 存到应用支持目录下的本地 key 文件，保证开发环境可以继续调试。Web/H5 仅用于预览，不保存 key，也不直连真实模型。
