...
桌面端点击左栏底部齿轮按钮进入“设置”，AI 模型只是其中一个分区。应用会把模型名、Base URL、Vault 位置和工作流偏好保存到应用支持目录下的 `synapse/settings.json`；`apiKey` 只保存到 macOS Keychain，不写入 settings/provider JSON，也不会创建明文本地 key 文件。Keychain 访问失败时应用 fail-closed，需要修复签名/entitlement 后重新构建，并由用户重新输入 key。Web/H5 仅用于预览，不保存 key，也不直连真实模型。
