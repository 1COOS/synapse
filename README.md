...
- 三栏学习工作台：左栏资源/搜索与大纲、中栏笔记阅读与源码编辑、右栏图片素材与 AI 建议；桌面端左右栏可折叠。
- 桌面端 Vault：首次选择并记住本地 Markdown 仓库目录。
- Web 预览库：使用内存示例数据，不直接访问本机文件系统。
- 项目模板：经文、书籍、学科、自定义。
- 笔记编辑：学习文本直接输入或粘贴到 Markdown 编辑区，剪贴板图片可直接粘贴为 Obsidian 友好的相对附件引用，并在预览中拖拽调整显示宽度或拖到另一张图片旁边并排。
...
桌面端点击左栏底部齿轮按钮进入“设置”，AI 模型只是其中一个分区。应用会把模型名、Base URL、Vault 位置和工作流偏好保存到应用支持目录下的 `synapse/settings.json`；`apiKey` 优先存到系统安全存储中，不写入 `settings.json`。如果 macOS 当前是无开发证书的未签名 Debug 构建，Keychain 可能返回 `-34018`，此时会降级把 key 存到应用支持目录下的本地 key 文件，保证开发环境可以继续调试。Web/H5 仅用于预览，不保存 key，也不直连真实模型。
