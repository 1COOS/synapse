...
- 稳定笔记身份：每篇笔记使用 UUID v4 `synapseId`；rename/move 保持 ID，copy 生成新 ID，旧 Vault 通过显式只读迁移升级。
- 文件事务：File Vault 使用 `.synapse/transactions` WAL，异常退出后自动回滚未提交的跨文件 mutation。
- 搜索缓存：macOS 左栏使用后台增量 SQLite 搜索，重启复用内容指纹；Web/H5 与 SQLite 不可用时降级为内存索引。
...
桌面端点击左栏底部齿轮按钮进入“设置”，AI 模型只是其中一个分区。应用会把模型名、Base URL、Vault 位置和工作流偏好保存到应用支持目录下的 `synapse/settings.json`；`apiKey` 只保存到 macOS Keychain，不写入 settings/provider JSON，也不会创建明文本地 key 文件。macOS Debug/Profile/Release 都必须使用有效 Apple Team 和证书签名；本机 Team ID 通过 ignored 的 `Signing.local.xcconfig` 注入。Web/H5 仅用于预览，不保存 key，也不直连真实模型。

设置面板固定分为 `通用 / AI 模型 / 搜索 / 外观 / 仓库 / 关于` 六个分区：

- 通用设置默认笔记模式、`250–10000ms` 自动保存延迟和 `120–2400px` 粘贴图片宽度；外观设置主题色和 `10–28px` 笔记字号。非法值会显示字段错误并阻止保存。
- AI 模型设置 Base URL、API Key、Chat/Vision Model；API Key 默认遮挡，明确确认后才允许清除。非空 Base URL 必须是绝对 `http/https` URL，配置不完整可以保存，但相关 AI 功能会标记为暂不可用。
- 搜索分区同时展示语义搜索意图和实际生效状态；没有 Embedding Model 时明确显示“未生效，仅使用全文搜索”。
- 仓库分区显示完整路径，可更换 Vault 或通过 Finder 定位目录；dirty 草稿切仓前必须选择保存、放弃或取消。
- 关于分区展示版本、构建号、平台模式、`settings.json` 路径、API Key 是否已配置和 fail-closed 说明；这里只展示已知状态，不主动读写或探测 Keychain。

设置使用统一草稿：只有草稿有效且相对基线有修改时才能保存，`⌘S`/`Ctrl+S` 可提交；取消、Esc 或关闭 dirty 草稿会要求确认。保存期间弹窗保持打开，失败信息完整可选择复制，只有保存 committed 后才关闭并显示“设置已保存”。桌面宽度使用侧边分类导航，紧凑宽度改为顶部分类；Web/H5 顶部明确显示“桌面端配置、Web 仅预览”，所有编辑、测试、保存和仓库操作均禁用。
...
设置面板按 Chat、Vision、Embedding 三项独立测试当前草稿，不会保存配置。Chat 发送短文本 `/chat/completions`；Vision 使用内置最小图片发送真实多模态请求；Embedding 使用固定短文本调用 `/embeddings`，并校验返回的是非空有限数值向量。界面会明确提示测试会产生真实请求，每项独立显示 loading、成功或完整错误。
...
用户内容以 Markdown Vault、附件和 `sources.json` 为真源。SQLite、向量索引、AI proposal 状态和其他 `.synapse-cache` 内容都应视为可删除、可重建数据。桌面端文件访问必须限制在用户选择的 Vault 根目录内；Web/H5 只作为沙盒预览。
