...
桌面端用户选择一个本机目录作为 Vault 根目录。Synapse 在该目录下读取和创建普通 Markdown 笔记与多层文件夹。Web/H5 使用浏览器内存示例库，只用于预览。

### 7.2 创建文件夹和笔记

左侧顶部提供两个 icon 操作：

- 资源树顶部的「Vault 根目录」用于切回最外层创建位置。
- 「新建文件夹」：在当前选中文件夹内创建子文件夹；选中笔记时在同级创建；选中 Vault 根目录时在最外层创建。
- 「新建笔记」：创建普通 `.md` 文件；选中文件夹时创建在文件夹内，选中笔记时创建在同级；选中 Vault 根目录时在最外层创建。
- 资源行支持删除。删除笔记会删除 `.md` 和同名 `.assets/`；删除文件夹会递归删除其中所有子文件夹、笔记和素材。
...
- **左栏：** 新建文件夹、新建笔记、Vault 根目录、资源树、资源删除、大纲树。
...
资源树不显示 `.assets/` 和 `.synapse-cache/`。删除笔记时同步删除同名 `.assets/`；删除文件夹时递归删除文件夹内所有资源。
