...
- 复制笔记会完整复制 Markdown、同名 `.assets/`、素材清单、附件和 proposal；复制出的 source/proposal 使用新的 id，并保持 proposal 指向复制后的 source。复制、重命名或冲突移动改变笔记文件名时，会同步调整正文中指向本笔记 assets 的图片路径。
...
资源树不显示 `.assets/` 和 `.synapse-cache/`。文件夹支持折叠、展开和递归笔记数量显示。重命名文件夹时同步移动其子文件夹、笔记和笔记素材目录。重命名或移动笔记时同步移动 `.md` 和同名 `.assets/`，并重写相关 source/proposal 的 `noteId`；文件名变化时同步改写正文中的本地 assets 图片路径。复制笔记时完整复制 Markdown、附件、sources 和 proposals，为复制出的 source/proposal 生成新 id，并让复制后的图片引用指向新 `.assets/`。删除笔记时同步删除同名 `.assets/`；删除文件夹时递归删除文件夹内所有资源。
...
- 从 Vault 完整重建素材清单和搜索索引的恢复任务尚未闭合。
