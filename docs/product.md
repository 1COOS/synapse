...
桌面端窗格只保留“编辑 / 阅读”两种视图。自动 PDF 分页默认关闭，打开笔记或进入编辑态时不读取分页附件快照、不生成 PDF。编辑态标题栏始终提供“显示分页线”按钮；用户为当前窗格当前笔记主动开启后，才按最终 PDF 的真实排版位置显示自动分页虚线和“第 N 页结束 / 第 N+1 页开始”，并展开窗格会话级纵向/横向切换。启用后正文停止变化 400 ms 自动重新分页，方向、附件和全局页面设置变化立即刷新；等待期间旧边界按当前 UTF-16 源码位置重定位并降低透明度。失败时保留旧边界并提供原位重试，没有旧结果时暂不显示。
...
自动分页线是不可命中、不可复制且隐藏语义的显示层，不写入 Markdown，也不改变光标、选区、复制或撤销。手动分页继续由 `<!-- synapse:page-break -->` 控制并使用现有可编辑辅助线，同一位置不重复绘制自动虚线。导出弹窗继承当前窗格方向和全局页面设置；在弹窗修改方向会同步回当前窗格，取消导出也不回滚。正式导出仍先 flush，只有正文、附件、方向、页边距和页脚状态全部与最后排版结果一致时才复用 PDF 字节。
