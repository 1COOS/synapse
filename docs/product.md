...
编辑和阅读视图都把当前窗格正文视为一篇可连续选择的文档。鼠标可正向或反向跨 block、跨屏拖选，双击选词、三击选段，`Shift` 点击继续扩展；双栏按“左栏全部 → 右栏全部 → 下方全宽”进入选区。普通复制始终输出对应的 Markdown 源码，包含所选范围内的 marker、pipe table、图片引用和布局注释；`⌘A`/`Ctrl+A` 一次选中当前窗格全部可见正文，不包含隐藏 frontmatter。
...
- 通用：默认笔记模式、`250–10000ms` 自动保存延迟、`120–2400px` 粘贴图片宽度，以及页面与 PDF 的 `10/15/20 mm` 页边距和页码页脚开关；
...
macOS 和 Windows 的每个笔记窗格标题栏提供“导出 PDF”，只导出该窗格当前笔记。打开导出弹窗前必须 flush 当前编辑会话；保存失败时不进入预览，编辑内容继续保留。成功后以不可变正文和本地图片快照生成 PDF，预览期间切换笔记或继续编辑不会改变本次导出。

导出固定使用 A4，弹窗只提供纵向/横向切换；页边距继承通用设置中的紧凑 10 mm、标准 15 mm、宽松 20 mm，默认标准 15 mm。页码页脚也继承全局开关，默认显示“当前页 / 总页数”；关闭后不生成页码并把页脚空间归还正文。弹窗显示真实分页缩略图、总页数和图片/表格降级警告；系统保存框取消不报错，写入失败保留弹窗以便重试。PDF 使用独立黑灰打印样式以及离线中文/粗体/代码/符号字体，每页页眉保持显示笔记标题。Web/H5 不显示入口。

桌面端窗格只保留“编辑 / 阅读”两种视图。自动 PDF 分页默认关闭，打开笔记或进入编辑态时不读取分页附件快照、不生成 PDF。编辑态标题栏始终提供“显示分页线”按钮；用户为当前窗格当前笔记主动开启后，才按最终 PDF 的真实排版位置显示自动分页虚线和“第 N 页结束 / 第 N+1 页开始”，并展开窗格会话级纵向/横向切换。启用后每次编辑事务立即投影未受影响的旧边界，被替换范围内的边界暂时隐藏；正文停止变化 400 ms 后通过不序列化 PDF 字节的权威排版校准，方向、附件和全局页面设置变化立即刷新。等待与失败期间保留的边界降低透明度，失败时提供原位重试，没有旧结果时暂不显示。

分页启用状态只属于当前 pane-note 会话：切到阅读态隐藏辅助线并暂停后台排版，返回同一笔记编辑态继续启用；切换笔记或关闭窗格后恢复默认关闭。再次关闭分页线会取消防抖与在途结果，后续编辑不计算；同一笔记重新开启时可直接恢复参数和快照完全匹配的缓存，否则立即重算。方向继续按 pane 会话记忆，不写入设置或笔记。

自动分页线是不可命中、不可复制且隐藏语义的显示层，不写入 Markdown，也不改变光标、选区、复制或撤销。手动分页继续由 `<!-- synapse:page-break -->` 控制并使用现有可编辑辅助线，同一位置不重复绘制自动虚线。导出弹窗继承当前窗格方向和全局页面设置；在弹窗修改方向会同步回当前窗格，取消导出也不回滚。正式导出仍先 flush，并始终针对不可变快照生成完整 PDF 字节；编辑态分页结果不携带可复用的 PDF 文件。
...
- **中栏：** macOS 使用同一个离线 CodeMirror 文档内核承载 Markdown 编辑与阅读，编辑态可按需开启真实 A4 分页辅助线，并提供手动保存和当前笔记 PDF 导出入口；Web/H5 暂时保留 Flutter 编辑器且不显示桌面分页能力。
...
- Web/H5 内存搜索缓存。
- macOS/Windows 编辑态按需开启并实时更新分页辅助线、当前笔记 A4 PDF 导出、真实分页预览、手动分页符和离线字体。
...
- 用户可以在编辑态主动开启与最终 A4 输出一致的分页边界，并按纵向或横向导出带页眉、可选页码页脚和手动分页控制的 PDF。
