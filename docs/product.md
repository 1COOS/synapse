...
左栏顶部提供资源列表和搜索两个 icon 模式，右侧提供折叠入口；桌面端左栏底部提供仓库选择和模型设置入口：

- 左侧资源树直接显示 Vault 下的顶层文件夹和笔记，不显示单独的根目录行。
- 资源模式内的「新建文件夹」和「新建笔记」固定在 Vault 根级创建资源。
- 搜索模式对当前 Vault 的 Markdown 笔记建立运行期内存索引，并在左栏显示结果；点击结果会打开对应笔记。
...
当前 UI 支持在左栏搜索整个 Vault 的 Markdown 笔记。搜索会使用运行期内存索引，并按笔记内容指纹增量更新；搜索结果结合全文命中和 Mock embedding 语义分数。SQLite 搜索缓存已经实现，但当前 UI 仍使用内存缓存。

## 8. 信息架构

### 8.1 主工作台

桌面宽度下采用三栏布局，macOS 内容区延伸到系统标题栏，左/中/右栏 icon 区与窗口缩放按钮处于同一行；Windows 和 Web/H5 使用同结构的 Flutter 顶部工具行降级，不模拟 macOS 窗口按钮。

- **左栏：** 资源/搜索模式切换、根级新建文件夹、根级新建笔记、Apple Notes 式资源树、文件夹和笔记右键菜单、大纲树、全仓搜索结果、底部仓库与模型设置入口。
- **中栏：** Markdown 阅读、Markdown 源码编辑、手动保存入口。
- **右栏：** 素材录入、素材选择、AI proposal 审核。

桌面端左右栏都可折叠为窄图标栏；左栏折叠后仍保留仓库选择和模型设置图标。
