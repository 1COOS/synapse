...
- **左栏：** 根级新建文件夹、根级新建笔记、Apple Notes 式资源树、文件夹右键菜单、大纲树。
...
资源树不显示 `.assets/` 和 `.synapse-cache/`。文件夹支持折叠、展开和递归笔记数量显示。删除笔记时同步删除同名 `.assets/`；删除文件夹时递归删除文件夹内所有资源。重命名文件夹时同步移动其子文件夹、笔记和笔记素材目录。
