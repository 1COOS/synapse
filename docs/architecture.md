...
`FileVaultBackend` 和 `MemoryVaultBackend` 保持原有 `VaultBackend` public API、构造方式和数据格式。两个 facade 分别委托 path、note、source、proposal 与 operations/state collaborators；parity tests 和 dispatch tests 约束两种实现的公共行为。File backend 的 Markdown、sidecar JSON 与新附件写入使用同目录临时文件、flush 和原子 rename，失败时保留原目标并清理临时文件。
...
当前基线为 `flutter test --no-pub` 639/639、`flutter analyze --no-pub` 无 issue。9 个超长测试文件已拆成 25 个，保留 248 tests 等价覆盖，当前最大测试文件 869 行。
