...
- Apple Development signing：执行 `xcodebuild test` 和 production entitlement gate 前，必须有有效 Apple Development signing certificate，并在 Xcode 中配置可用 Team；
- Chrome：Web/H5 内存预览；
...
`xcodebuild test`、签名构建和最终 entitlement 检查的前置条件是本机存在有效 Apple Development signing certificate，且 Runner 已配置可用 Team。缺失证书、Team 或签名权限属于外部门禁阻塞，不等于 Dart/Flutter 代码失败；此时应分别报告已完成的代码验证和未完成的签名 gate，不得宣称完整 production gate 已通过。
