...
当前记录基线为 633/633 tests、analyze no issues。9 个超长测试文件已拆为 25 个，保留 248 tests 等价覆盖，最大测试文件 869 行。该记录不是最终 production gate 结果。
...
只有上述命令全部通过且工作区边界清楚后，才能宣称最终 macOS production gate 通过。文档同步、既有 633 tests 基线或单独的 `flutter analyze` 都不能替代该结论。
