...
- Apple Development signing：执行 `xcodebuild test` 和带 entitlements 的 Debug/Release 签名构建前，必须有有效 Apple Development signing certificate，并在 Xcode 中配置可用 Team；`codesign -d` 检查本身不需要证书，但需要前一步成功生成可检查的 app；
- Chrome：Web/H5 内存预览；
...
`xcodebuild test` 和带 entitlements 的 Debug/Release 签名构建要求本机存在有效 Apple Development signing certificate，且 Runner 已配置可用 Team。`codesign -d` 检查本身不需要证书，但需要前一步成功生成可检查的 app。当前缺失证书、Team 或签名权限时属于外部门禁阻塞，不等于 Dart/Flutter 代码失败；此时应分别报告已完成的代码验证和未完成的签名 gate，不得宣称完整 production gate 已通过。
